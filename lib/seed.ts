...
export function createInitialState(withDemo = false): AppState {
  ...
  const logs: DailyLog[] = withDemo ? topics.map((topic, index) => ({
    ...
    title: topic[0], assignedWork: topic[0],
    actionsTaken: index === 0 ? "Đọc hướng dẫn, quan sát cấu trúc hồ sơ và cách đánh tham chiếu." : "Đối chiếu dữ liệu được giao với chứng từ và ghi nhận kết quả trên giấy làm việc.",
    relatedDocuments: "Hồ sơ và chứng từ đã được ẩn thông tin nhận diện", tools: "Microsoft Excel",
    appliedKnowledge: "Kiến thức kiểm toán, kỹ năng đối chiếu và lập hồ sơ",
    result: "Hoàn thành phần việc được giao và gửi người hướng dẫn soát xét.",
    difficulties: "Chưa quen cách trình bày giấy làm việc.", resolution: "Tham khảo mẫu và trao đổi với người hướng dẫn.",
    lessonsLearned: "Cần lưu vết đầy đủ cho từng kết luận kiểm toán.", additionalNotes: "Dữ liệu minh hoạ — có thể xoá trong Cài đặt.",
    workType: index === 0 ? "Tìm hiểu quy trình" : "Thủ tục kiểm toán",
    tags: index === 1 ? ["TK 152", "Lấy mẫu kiểm toán", "Excel"] : index === 2 ? ["TK 211", "Giấy làm việc"] : index === 3 ? ["TK 242", "Phân bổ chi phí"] : ["Thủ tục kiểm toán"],
    files: [], sensitive: false, createdAt: now(), updatedAt: now(),
  })) : [];
  return {
    version: 1,
    ...
    conclusion: {
      id: id(), internshipId,
      rows: plans.map(plan => ({ week: plan.week, plannedTarget: plan.target, actualTarget: "", plannedWork: plan.workContent, actualWork: "", limitations: "", correctiveSolution: "", solutionExecution: "" })),
      completedWork: "", professionalKnowledge: "", developedSkills: "", lessons: "", personalLimitations: "", personalChanges: "", internshipValue: "", finalConclusion: "", updatedAt: now(),
    },
    references: [], appendices: [], settings: { theme: "light", autosave: true, aiConsentRequired: true, defaultAnonymization: true },
  };
}
