...
function Settings({ state, setState, saveState }: { state: AppState; setState: (s: AppState) => void; saveState: (s: AppState) => Promise<boolean> }) {
  const [form, setForm] = useState(() => ({ fullName: state.profile.fullName, organization: state.internship.organization, startDate: state.internship.startDate, endDate: state.internship.endDate }));
  const [saving, setSaving] = useState(false), [saved, setSaved] = useState(false), [error, setError] = useState("");
  const original = { fullName: state.profile.fullName, organization: state.internship.organization, startDate: state.internship.startDate, endDate: state.internship.endDate };
  const dirty = JSON.stringify(form) !== JSON.stringify(original);
  const change = (patch: Partial<typeof form>) => { setForm(current => ({ ...current, ...patch })); setSaved(false); setError(""); };
  const submit = async () => {
    const cleaned = { ...form, fullName: form.fullName.trim(), organization: form.organization.trim() };
    ...
    const nextState = updateInternship({ ...state, profile: { ...state.profile, fullName: cleaned.fullName } }, { organization: cleaned.organization, startDate: cleaned.startDate, endDate: cleaned.endDate });
