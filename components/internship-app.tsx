...
function DashboardSchedule({ state, update }: { state: AppState; update: (fn: (s: AppState) => AppState) => void }) {
  ...
  return <section className="dashboard-schedule"><div className="internship-info"><span>▦</span><div><small>THÔNG TIN THỰC TẬP</small><b>{state.internship.organization || "Chưa thiết lập đơn vị thực tập"}</b></div></div><Field label="Đơn vị thực tập"><input type="text" value={state.internship.organization} placeholder="Nhập tên công ty/đơn vị" maxLength={120} onChange={e => update(s => ({ ...s, internship: { ...s.internship, organization: e.target.value } }))} /></Field><Field label="Ngày bắt đầu"><input type="date" value={state.internship.startDate} max={state.internship.endDate || undefined} onChange={e => setDate("startDate", e.target.value)} /></Field><Field label="Ngày kết thúc"><input type="date" value={state.internship.endDate} min={state.internship.startDate || undefined} onChange={e => setDate("endDate", e.target.value)} /></Field><Link className="primary-btn add-log-btn" href="/nhat-ky">＋ Thêm nhật ký</Link></section>;
}
