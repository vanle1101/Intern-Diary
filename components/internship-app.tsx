...
export default function InternshipApp({ view }: { view: View }) {
  ...
  <main><header className="topbar"><button className="menu-btn" onClick={() => setMenu(true)} aria-label="Mở trình đơn">☰</button><div className="crumb">Không gian làm việc <span>/</span> {nav.find(n => n.view === view)?.label}</div><div className={status === "saved" ? "save saved" : status === "error" ? "save error" : "save"}><i />{status === "saved" ? "Đã lưu" : status === "error" ? "Lỗi lưu dữ liệu" : "Đang lưu…"}</div><span className="account-name">{user?.username}</span><button className="logout-btn" onClick={() => void logout()}>Đăng xuất</button></header>
