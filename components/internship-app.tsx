...
export default function InternshipApp({ view }: { view: View }) {
  ...
  if (status === "locked") return <div className="access-page"><section className="access-intro locked-art" aria-label="Nhật ký thực tập UEH" style={{ backgroundImage: "url('/auth-ueh-left-fixed.png')" }} onContextMenu={e => e.preventDefault()}><img className="locked-art-image" src="/auth-ueh-left-fixed.png" alt="" draggable={false} aria-hidden="true" /></section><form className="access-card" aria-busy={authWorking} onSubmit={e => { e.preventDefault(); void submitAuth(); }}><div className="access-form"><small className="auth-kicker">NHẬT KÝ THỰC TẬP</small><i className="auth-rule" /><h1>{isRegister ? "Tạo tài khoản nhật ký" : "Chào mừng bạn quay lại!"}</h1><p>{isRegister ? "Đăng ký để bắt đầu quản lý nhật ký thực tập cá nhân." : "Đăng nhập để tiếp tục nhật ký thực tập"}</p>{isRegister && <label><span>Họ và tên</span><div className="auth-input"><em aria-hidden="true">♙</em><input disabled={authWorking} autoComplete="name" placeholder="Nhập họ và tên của bạn" value={fullName} onChange={e => setFullName(e.target.value)} minLength={2} maxLength={80} required /></div></label>}<label><span>Tên đăng nhập</span><div className="auth-input"><em aria-hidden="true">♙</em><input disabled={authWorking} autoComplete="username" placeholder="Nhập tên đăng nhập" value={username} onChange={e => setUsername(e.target.value)} minLength={3} maxLength={32} pattern="[A-Za-z0-9._-]+" required /></div></label><label><span>Mật khẩu</span><div className="auth-input"><em aria-hidden="true">▣</em><input disabled={authWorking} type="password" autoComplete={isRegister ? "new-password" : "current-password"} placeholder="Nhập mật khẩu" value={password} onChange={e => setPassword(e.target.value)} minLength={6} maxLength={128} required /><em aria-hidden="true">◉</em></div></label>{!isRegister && <div className="auth-row"><label className="remember"><input type="checkbox" disabled={authWorking} />Ghi nhớ đăng nhập</label></div>}{message && <div className="access-error">{message}</div>}<button className="primary-btn auth-submit" disabled={authWorking}>{isRegister ? "Tạo tài khoản" : "Đăng nhập"}</button><button type="button" className="auth-mini-switch" disabled={authWorking} onClick={() => { setAuthMode(isRegister ? "login" : "register"); setMessage(""); }}>{isRegister ? "Quay lại đăng nhập" : "Tạo tài khoản mới"}</button></div><footer><strong>Pay for Pass</strong><nav><span>Dự án cá nhân</span><span>Nhật ký thực tập UEH</span></nav></footer></form></div>;
