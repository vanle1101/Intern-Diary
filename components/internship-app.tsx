...
export default function InternshipApp({ view }: { view: View }) {
  ...
  if (status === "locked") return <div className="access-page"><form className="access-card" onSubmit={e => { e.preventDefault(); if (accessInput.trim()) void unlock(accessInput.trim()); }}><span className="brand-mark">N</span><small>NHẬT KÝ THỰC TẬP UEH</small><h1>Mở dữ liệu trên Vercel</h1><p>Nhập mã truy cập cá nhân. Nội dung nhật ký được lưu trên đám mây, không lưu trong bộ nhớ trình duyệt.</p><label><span>Mã truy cập</span><input type="password" autoComplete="current-password" value={accessInput} onChange={e => setAccessInput(e.target.value)} required /></label>{message && <div className="access-error">{message}</div>}<button className="primary-btn">Mở nhật ký</button></form></div>;
...
function Dashboard({ state }: { state: AppState }) {
  ...
  const week = Math.min(12, Math.max(1, Math.ceil((currentTime - new Date(state.internship.startDate).getTime()) / 604800000)));
  const progress = getProgress(state), checks = getComplianceItems(state), reached = checks.filter(item => item.status === "Đạt").length;
  return <><Title eyebrow="THỰC TẬP TỐT NGHIỆP" title={`Xin chào, ${state.profile.fullName} 👋`} desc="Theo dõi tiến độ và hoàn thiện nhật ký thực tập của bạn." action={<Link className="primary-btn" href="/nhat-ky">＋ Thêm nhật ký</Link>} />
  <section className="hero"><div className="ring" style={{ "--p": `${progress.overall * 3.6}deg` } as React.CSSProperties}><span>{progress.overall}<small>%</small></span></div><div><small>TIẾN ĐỘ TỔNG THỂ</small><h2>Bạn đang đi đúng hướng</h2><p>Tiếp tục cập nhật nhật ký hằng ngày để hệ thống tổng hợp chính xác.</p><div className="milestones"><span className={progress.planComplete === 12 ? "done" : "current"}>1 Kế hoạch</span><span className={state.activities.length >= 3 ? "done" : "current"}>2 Hoạt động chính</span><span className={progress.conclusionFields === 8 ? "done" : "current"}>3 Kết luận</span></div></div><div className="week-card"><small>TUẦN HIỆN TẠI</small><strong>{week}<em>/12</em></strong><div><span style={{ width: `${week / 12 * 100}%` }} /></div><p>{Math.max(0, 12 - week)} tuần còn lại</p></div></section>
...
function LogModal({ log, onClose, onSave }: { log: DailyLog; onClose: () => void; onSave: (l: DailyLog) => void }) {
  ...
  return <Modal title={log.title ? "Chỉnh sửa bản ghi" : "Thêm nhật ký mới"} onClose={onClose} onSubmit={() => form.title.trim() && onSave({ ...form, title: form.title.trim(), tags: Array.from(new Set(form.tags.map(tag => tag.trim()).filter(Boolean))) })} footer="Tự động lưu trên Vercel sau khi xác nhận"><div className="form-grid">
    ...
    <Sensitive checked={form.sensitive} onChange={v => set("sensitive", v)} />
  </div></Modal>;
}
...
function Compliance({ state }: { state: AppState }) { const progress = getProgress(state), checks = getComplianceItems(state); return <><Title eyebrow="UEH 2026" title="Kiểm tra yêu cầu" desc="Công cụ hỗ trợ rà soát, không thay thế việc tự kiểm tra quy định trước khi nộp." /><div className="notice">⚠ Số trang được ước tính theo 500 từ mỗi trang: Phần 1 có {progress.part1Pages} trang, Phần 2 có {progress.part2Pages} trang, Phần 3 có {progress.part3Pages} trang. Trạng thái “Đạt” không đồng nghĩa hồ sơ đã được Khoa chấp thuận.</div><div className="checks">{checks.map(item => <div key={item.label}><span className={item.status === "Đạt" ? "ok" : "pending"}>{item.status === "Đạt" ? "✓" : item.status === "Cần kiểm tra" ? "?" : "!"}</span><b>{item.label}</b><em>{item.status}</em></div>)}</div></>; }
function Settings({ state, setState }: { state: AppState; setState: (s: AppState) => void }) { return <><Title eyebrow="CÁ NHÂN HOÁ" title="Cài đặt" desc="Quản lý dữ liệu trên Vercel và các nguyên tắc bảo mật." /><div className="settings"><section className="panel"><h3>Hồ sơ thực tập</h3><Field label="Họ và tên"><input value={state.profile.fullName} onChange={e => setState({ ...state, profile: { ...state.profile, fullName: e.target.value } })} /></Field><Field label="Đơn vị thực tập"><input value={state.internship.organization} onChange={e => setState({ ...state, internship: { ...state.internship, organization: e.target.value } })} /></Field></section><section className="panel"><h3>Dữ liệu trên Vercel</h3><p>Mọi thay đổi được tự động lưu vào kho Blob riêng tư trên Vercel. Trình duyệt không giữ bản sao nội dung nhật ký.</p><button className="secondary-btn" onClick={() => confirm("Thay dữ liệu hiện tại bằng dữ liệu minh hoạ?") && setState(resetState(true))}>Nạp dữ liệu minh hoạ</button><button className="danger-btn" onClick={() => confirm("Xoá toàn bộ nội dung nhật ký trên Vercel?") && setState(resetState(false))}>Xoá dữ liệu và bắt đầu trống</button></section></div></>; }
...
function Sensitive({ checked, onChange }: { checked: boolean; onChange: (v: boolean) => void }) { return <label className="sensitive-check wide"><input aria-label="Đánh dấu dữ liệu nhạy cảm" type="checkbox" checked={checked} onChange={e => onChange(e.target.checked)} /><span><b>Đánh dấu dữ liệu nhạy cảm</b><small>Yêu cầu ẩn danh trước khi xuất hoặc dùng AI.</small></span></label>; }
