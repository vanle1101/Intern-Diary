...
const mobileNav = nav.filter(item => ["dashboard", "logs", "plan", "activities", "settings"].includes(item.view));
...
export default function InternshipApp({ view }: { view: View }) {
  ...
  const saveSettings = async (nextState: AppState) => {
    setStatus("saving");
    try {
      if (!isLocalPreviewUser) await persistCloudState(nextState);
      skipInitialSave.current = true;
  ...
  return <div className="app-shell"><style jsx global>{`
    ...
    .admin-summary{display:flex!important;align-items:center!important;gap:8px!important;padding:12px 16px!important;border:1px solid #dce9e5!important;border-radius:12px!important;background:#fff!important}
    .admin-summary b{color:#006f58!important;font-size:23px!important}.admin-summary b.online-number{color:#16a269!important}.admin-summary span{color:#687973!important;font-size:13px!important;margin-right:10px!important}
    .admin-users{padding:0!important;overflow:hidden!important}.admin-user-list{display:flex!important;flex-direction:column!important}
    .admin-user-list article{min-height:76px!important;display:grid!important;grid-template-columns:14px minmax(180px,1fr) auto minmax(150px,.35fr)!important;gap:16px!important;align-items:center!important;padding:15px 22px!important;border-bottom:1px solid #e8efec!important}
    .admin-user-list article>div b,.admin-user-list article>div small{display:block!important}.admin-user-list article>div b{font-size:16px!important}.admin-user-list article>div small{margin-top:4px!important;color:#75847e!important;font-size:13px!important}
    .admin-user-list article>em{padding:6px 9px!important;border-radius:999px!important;background:#fff0e4!important;color:#c35a1b!important;font-size:10px!important;font-style:normal!important;font-weight:900!important}.admin-user-list article>time{justify-self:end!important;color:#71817a!important;font-size:13px!important}
    .presence-dot{width:11px!important;height:11px!important;border-radius:50%!important;background:#c3cec9!important}.presence-dot.online{background:#19a66a!important;box-shadow:0 0 0 5px rgba(25,166,106,.1)!important}
    @media(max-width:760px){.online-widget{margin-top:24px!important}.account-cluster{gap:8px!important}.admin-badge{display:none!important}.admin-summary{display:grid!important;grid-template-columns:auto 1fr!important}.admin-user-list article{grid-template-columns:14px 1fr auto!important;gap:10px!important;padding:14px!important}.admin-user-list article>time{grid-column:2/-1!important;justify-self:start!important}}
  `}</style><aside className={menu ? "sidebar open" : "sidebar"}>
    ...
    {user?.role === "admin" ? <Link className="online-widget" href="/nguoi-dung" aria-label={`Có ${onlineCount} người đang online`}><span aria-hidden="true" /><b>{onlineCount} người đang online</b></Link> : <div className="online-widget" aria-label={`Có ${onlineCount} người đang online`}><span aria-hidden="true" /><b>{onlineCount} người đang online</b></div>}
  </aside>{menu && <button className="scrim" onClick={() => setMenu(false)} aria-label="Đóng menu" />}
  <main><header className="topbar"><button className="menu-btn" onClick={() => setMenu(true)} aria-label="Mở trình đơn">☰</button><div className="crumb">Không gian làm việc <span>/</span> {visibleNav.find(n => n.view === view)?.label}</div><div className="account-cluster"><span className="account-name">{user?.username}</span>{user?.role === "admin" && <span className="admin-badge">ADMIN</span>}<span className="user-avatar">{initials || "LH"}</span><button className="logout-btn" onClick={() => void logout()}>Đăng xuất</button></div></header>
  <div className="page">{view === "dashboard" && <Dashboard state={state} />}{view === "logs" && <Logs state={state} update={update} />}{view === "plan" && <Plan state={state} update={update} />}{view === "activities" && <Activities state={state} update={update} />}{view === "conclusion" && <ConclusionView state={state} update={update} />}{view === "compliance" && <Compliance state={state} />}{view === "settings" && <Settings state={state} setState={setState} saveState={saveSettings} />}{view === "users" && (user?.role === "admin" ? <AdminUsers preview={isLocalPreviewUser} onOnlineCount={setOnlineCount} /> : <AccessDenied />)}</div><footer className="app-footer"><span>Pay to Pass · Dự án cá nhân · Nhật ký thực tập UEH</span></footer></main><nav className="mobile-bottom-nav" aria-label="Điều hướng nhanh">{mobileNav.map(item => <Link key={item.view} href={item.href} className={view === item.view ? "active" : ""} aria-current={view === item.view ? "page" : undefined}><MobileNavIcon view={item.view} /><span>{item.view === "dashboard" ? "Trang chủ" : item.view === "logs" ? "Nhật ký" : item.view === "plan" ? "Kế hoạch" : item.view === "activities" ? "Hoạt động" : "Cài đặt"}</span></Link>)}</nav><AssistantAgent state={state} view={view} navigate={href => { router.push(href); setMenu(false); }} /></div>;
}
...
function MobileNavIcon({ view }: { view: View }) {
  const paths: Partial<Record<View, React.ReactNode>> = {
    ...
    logs: <><path d="M5 3h11l3 3v15H5z" /><path d="M8 11h8M8 15h8M8 7h4" /></>,
    plan: <><rect x="3" y="5" width="18" height="16" rx="2" /><path d="M16 3v4M8 3v4M3 10h18" /></>,
    activities: <><path d="M12 3 3.5 8 12 13l8.5-5z" /><path d="m5 12 7 4 7-4M5 16l7 4 7-4" /></>,
    settings: <><circle cx="12" cy="12" r="3" /><path d="M19.4 15a1.7 1.7 0 0 0 .34 1.88l.06.06-2.83 2.83-.06-.06A1.7 1.7 0 0 0 15 19.4a1.7 1.7 0 0 0-1 .6 1.7 1.7 0 0 0-.4 1.1V21h-4v-.09A1.7 1.7 0 0 0 8.6 19.4a1.7 1.7 0 0 0-1.88.34l-.06.06-2.83-2.83.06-.06A1.7 1.7 0 0 0 4.6 15a1.7 1.7 0 0 0-.6-1 1.7 1.7 0 0 0-1.1-.4H3v-4h.09A1.7 1.7 0 0 0 4.6 8.6a1.7 1.7 0 0 0-.34-1.88l-.06-.06 2.83-2.83.06.06A1.7 1.7 0 0 0 9 4.6a1.7 1.7 0 0 0 1-.6 1.7 1.7 0 0 0 .4-1.1V3h4v.09A1.7 1.7 0 0 0 15.4 4.6a1.7 1.7 0 0 0 1.88-.34l.06-.06 2.83 2.83-.06.06A1.7 1.7 0 0 0 19.4 9c.16.37.37.71.66 1 .3.28.68.43 1.09.43H21v4h-.09A1.7 1.7 0 0 0 19.4 15Z" /></>,
  };
...
function Dashboard({ state }: { state: AppState }) {
  ...
  const overall = Math.round((Math.min(1, progress.planComplete / 12) + Math.min(1, state.activities.length / 3) + Math.min(1, progress.conclusionFields / 8)) / 3 * 100);
  const recentLogs = [...state.dailyLogs].sort((a, b) => (b.date || b.updatedAt).localeCompare(a.date || a.updatedAt)).slice(0, 3);
  return <><section className="dashboard-welcome"><div><small>CHÀO MỪNG BẠN ĐẾN VỚI</small><h1>Nhật ký thực tập <span>👋</span></h1><p>Ghi lại hành trình thực tập, phát triển kỹ năng và hoàn thiện báo cáo thực tập cùng UEH.</p><div className="dashboard-mobile-progress"><span><b>Tiến độ hồ sơ</b><strong>{overall}%</strong></span><i><em style={{ width: `${overall}%` }} /></i></div></div><blockquote>“Kiến thức tạo ra giá trị.<br />Hành động tạo nên sự khác biệt.”<small>– UEH</small></blockquote></section>
  <DashboardSchedule state={state} />
  <section className="topic-hero"><span aria-hidden="true">▤</span><div><small>TÊN ĐỀ TÀI</small><h2>{state.internship.topicName || "Chưa thiết lập tên đề tài"}</h2>{!state.internship.topicName && <Link href="/cai-dat">Nhập tên đề tài trong Cài đặt</Link>}</div></section>
  ...
  <section className="recent-diary panel"><Head title="Nhật ký gần đây" sub="Các ghi chép mới nhất trong kỳ thực tập" />{recentLogs.length ? <div className="recent-diary-list">{recentLogs.map(log => <Link href="/nhat-ky" key={log.id}><time dateTime={log.date}>{formatInternshipDate(log.date)}</time><div><b>{log.title}</b><small>Tuần {log.week} · {log.workType || "Chưa phân loại"}</small></div><span>›</span></Link>)}</div> : <div className="recent-diary-empty"><span aria-hidden="true">✎</span><div><b>Chưa có nhật ký nào</b><small>Ghi lại công việc đầu tiên của bạn hôm nay.</small></div><Link href="/nhat-ky">Thêm ngay</Link></div>}</section>
...
function Plan({ state, update }: { state: AppState; update: (fn: (s: AppState) => AppState) => void }) {
  ...
  const fields: [keyof InternshipPlan, string][] = [["workContent", "Nội dung công việc"], ["target", "Mục tiêu cần đạt"], ["method", "Phương pháp sử dụng"], ["supportRequired", "Yêu cầu hỗ trợ"], ["expectedResult", "Yêu cầu kết quả"]];
  return <><Title eyebrow="PHẦN 1" title="Kế hoạch thực tập" desc="Kế hoạch được giữ nguyên để đối chiếu với kết quả thực tế, không tự ghi đè." action={<span className="status-pill">{state.plans.filter(p => p.workContent).length}/12 tuần</span>} /><div className="table-wrap plan-table"><table><thead><tr><th>Tuần</th>{fields.map(([, label]) => <th key={label}>{label}</th>)}</tr></thead><tbody>{state.plans.map(p => <tr key={p.id}><th><span>Tuần</span>{p.week}</th>{fields.map(([key, label]) => { const value = String(p[key]); return <td key={key} data-label={label}><TranslateButton text={value} onTranslated={translated => setPlan(p.id, key, translated)} /><textarea aria-label={`${label} tuần ${p.week}`} value={value} onChange={e => setPlan(p.id, key, e.target.value)} placeholder="Nhập nội dung…" /></td>; })}</tr>)}</tbody></table></div></>;
}
...
function Settings({ state, setState, saveState }: { state: AppState; setState: (s: AppState) => void; saveState: (s: AppState) => Promise<boolean> }) {
  const [form, setForm] = useState(() => ({ fullName: state.profile.fullName, organization: state.internship.organization, topicName: state.internship.topicName, startDate: state.internship.startDate, endDate: state.internship.endDate }));
  const [saving, setSaving] = useState(false), [saved, setSaved] = useState(false), [error, setError] = useState("");
  const original = { fullName: state.profile.fullName, organization: state.internship.organization, topicName: state.internship.topicName, startDate: state.internship.startDate, endDate: state.internship.endDate };
  const dirty = JSON.stringify(form) !== JSON.stringify(original);
  const change = (patch: Partial<typeof form>) => { setForm(current => ({ ...current, ...patch })); setSaved(false); setError(""); };
  const submit = async () => {
    const cleaned = { ...form, fullName: form.fullName.trim(), organization: form.organization.trim(), topicName: form.topicName.trim() };
    if (cleaned.startDate && cleaned.endDate && cleaned.endDate < cleaned.startDate) { setError("Ngày kết thúc phải sau ngày bắt đầu."); return; }
    setSaving(true); setError("");
    const nextState = updateInternship({ ...state, profile: { ...state.profile, fullName: cleaned.fullName } }, { organization: cleaned.organization, topicName: cleaned.topicName, startDate: cleaned.startDate, endDate: cleaned.endDate });
    const ok = await saveState(nextState);
    setSaving(false);
    if (ok) { setForm(cleaned); setSaved(true); } else setError("Không thể lưu lúc này. Vui lòng thử lại.");
  };
  return <><Title eyebrow="CÁ NHÂN HOÁ" title="Cài đặt" desc="Quản lý hồ sơ và dữ liệu của bạn." /><div className="settings"><form className="panel" onSubmit={e => { e.preventDefault(); void submit(); }}><h3>Hồ sơ thực tập</h3><Field label="Họ và tên"><input value={form.fullName} onChange={e => change({ fullName: e.target.value })} /></Field><Field label="Đơn vị thực tập"><input value={form.organization} placeholder="Nhập tên công ty/đơn vị" onChange={e => change({ organization: e.target.value })} /></Field><Field label="Tên đề tài"><input value={form.topicName} placeholder="Nhập tên đề tài thực tập" maxLength={250} onChange={e => change({ topicName: e.target.value })} /></Field><Field label="Ngày bắt đầu"><input type="date" value={form.startDate} max={form.endDate || undefined} onChange={e => change({ startDate: e.target.value })} /></Field><Field label="Ngày kết thúc"><input type="date" value={form.endDate} min={form.startDate || undefined} onChange={e => change({ endDate: e.target.value })} /></Field>{error && <p className="settings-save-error">{error}</p>}<div className="settings-save-row"><button className="primary-btn" disabled={saving || (!dirty && !saved)}>{saving ? "Đang lưu…" : saved && !dirty ? "✓ Đã lưu" : "Lưu cài đặt"}</button></div></form><section className="panel"><h3>Dữ liệu nhật ký</h3><p>Dữ liệu hồ sơ chỉ được cập nhật khi bạn bấm “Lưu cài đặt”.</p><button className="danger-btn clear-data-btn" onClick={() => confirm("Xoá toàn bộ nội dung nhật ký?") && setState(resetState(false))}>Xoá dữ liệu và bắt đầu trống</button></section></div></>;
}
