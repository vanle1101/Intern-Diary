...
export default function InternshipApp({ view }: { view: View }) {
  ...
  if (status === "locked") return <div className="access-page"><section className="access-intro locked-art" aria-label="Nhật ký thực tập UEH" onContextMenu={e => e.preventDefault()}><img className="locked-art-image" src="/auth-ueh-left.png?v=locked-v2" alt="" draggable={false} aria-hidden="true" /></section><form className="access-card" aria-busy={authWorking} onSubmit={e => { e.preventDefault(); void submitAuth(); }}><div className="access-form"><small className="auth-kicker">NHẬT KÝ THỰC TẬP</small><i className="auth-rule" /><h1>{isRegister ? "Tạo tài khoản nhật ký" : "Chào mừng bạn quay lại!"}</h1><p>{isRegister ? "Đăng ký để bắt đầu quản lý nhật ký thực tập cá nhân." : "Đăng nhập để tiếp tục nhật ký thực tập"}</p>{isRegister && <label><span>Họ và tên</span><div className="auth-input"><em aria-hidden="true">♙</em><input disabled={authWorking} autoComplete="name" placeholder="Nhập họ và tên của bạn" value={fullName} onChange={e => setFullName(e.target.value)} minLength={2} maxLength={80} required /></div></label>}<label><span>Mã số sinh viên / Email</span><div className="auth-input"><em aria-hidden="true">♙</em><input disabled={authWorking} autoComplete="username" placeholder="Nhập mã số sinh viên hoặc email" value={username} onChange={e => setUsername(e.target.value)} minLength={3} maxLength={80} pattern="[A-Za-z0-9._@-]+" required /></div></label><label><span>Mật khẩu</span><div className="auth-input"><em aria-hidden="true">▣</em><input disabled={authWorking} type="password" autoComplete={isRegister ? "new-password" : "current-password"} placeholder="Nhập mật khẩu" value={password} onChange={e => setPassword(e.target.value)} minLength={6} maxLength={128} required /><em aria-hidden="true">◉</em></div></label>{!isRegister && <div className="auth-row"><label className="remember"><input type="checkbox" disabled={authWorking} />Ghi nhớ đăng nhập</label><button type="button" className="auth-link" onClick={() => setMessage("Bạn có thể liên hệ Khoa/Viện quản lý thực tập để được hỗ trợ.")}>Quên mật khẩu?</button></div>}{message && <div className="access-error">{message}</div>}<button className="primary-btn auth-submit" disabled={authWorking}>{isRegister ? "Tạo tài khoản" : "Đăng nhập"}</button>{!isRegister && <><div className="auth-divider"><span />hoặc<span /></div><button type="button" className="auth-ms" disabled={authWorking} onClick={() => setMessage("Tính năng Microsoft chưa được cấu hình cho dự án cá nhân này.")}><span className="ms-logo" aria-hidden="true"><i /><i /><i /><i /></span>Đăng nhập với Microsoft</button><p className="auth-contact">Chưa có tài khoản? <button type="button" onClick={() => { setAuthMode("register"); setMessage(""); }}>Liên hệ Khoa/Viện quản lý thực tập</button></p></>}{isRegister && <><div className="auth-divider"><span />hoặc<span /></div><button type="button" className="auth-ms" disabled={authWorking} onClick={() => { setAuthMode("login"); setMessage(""); }}>Quay lại đăng nhập</button></>}</div><footer>© 2026 UEH. All rights reserved.<nav><span>Điều khoản sử dụng</span><span>Chính sách bảo mật</span><span>Hướng dẫn</span></nav></footer></form></div>;
  ...
  return <div className="app-shell"><aside className={menu ? "sidebar open" : "sidebar"}>
    <Link className="brand ueh-sidebar-logo locked-art" href="/" onContextMenu={e => e.preventDefault()}><img src="/ueh-wordmark.png?v=locked-v2" alt="UEH University" draggable={false} /></Link>
