...
export default function InternshipApp({ view }: { view: View }) {
  ...
  if (status === "locked") return <div className="access-page"><section className="access-intro"><div className="access-brand ueh-wordmark"><div><b>UEH</b><strong>UNIVERSITY</strong></div><span>Đại học Kinh tế<br />Thành phố Hồ Chí Minh<small>University of Economics<br />Ho Chi Minh City</small></span></div><div className="access-copy"><h2><span>NHẬT KÝ</span><strong>THỰC TẬP UEH</strong></h2><i /><p>Lưu lại hành trình thực tập, phát triển kỹ năng và hoàn thiện bản thân mỗi ngày.</p><div className="access-highlights"><article><em>▣</em><strong>Theo dõi hằng ngày</strong><span>Dễ dàng ghi chép và quản lý tiến độ</span></article><article><em>▤</em><strong>Báo cáo tổng quan</strong><span>Thống kê và đánh giá quá trình thực tập</span></article><article><em>▥</em><strong>Xuất báo cáo</strong><span>Tạo báo cáo chuyên nghiệp nhanh chóng</span></article></div></div><div className="campus-scene" aria-hidden="true"><span /><span /><span /></div><p className="access-motto">“Học để làm – Làm để dẫn đầu – Lead the Change”</p></section><form className="access-card" aria-busy={authWorking} onSubmit={e => { e.preventDefault(); void submitAuth(); }}><div className="access-form"><div className="ueh-wordmark form-logo"><div><b>UEH</b><strong>UNIVERSITY</strong></div><span>Đại học Kinh tế<br />Thành phố Hồ Chí Minh<small>University of Economics<br />Ho Chi Minh City</small></span></div><h1>{isRegister ? "Tạo tài khoản nhật ký" : "Chào mừng bạn quay lại!"}</h1><p>{isRegister ? "Đăng ký để bắt đầu quản lý nhật ký thực tập cá nhân." : "Đăng nhập để tiếp tục nhật ký thực tập"}</p>{isRegister && <label><span>Họ và tên</span><div className="auth-input"><em aria-hidden="true">◎</em><input disabled={authWorking} autoComplete="name" placeholder="Nhập họ và tên của bạn" value={fullName} onChange={e => setFullName(e.target.value)} minLength={2} maxLength={80} required /></div></label>}<label><span>Mã số sinh viên / Email</span><div className="auth-input"><em aria-hidden="true">♙</em><input disabled={authWorking} autoComplete="username" placeholder="Nhập mã số sinh viên hoặc email" value={username} onChange={e => setUsername(e.target.value)} minLength={3} maxLength={80} pattern="[A-Za-z0-9._@-]+" required /></div></label><label><span>Mật khẩu</span><div className="auth-input"><em aria-hidden="true">▣</em><input disabled={authWorking} type="password" autoComplete={isRegister ? "new-password" : "current-password"} placeholder="Nhập mật khẩu" value={password} onChange={e => setPassword(e.target.value)} minLength={6} maxLength={128} required /></div></label>{!isRegister && <div className="auth-row"><label className="remember"><input type="checkbox" disabled={authWorking} />Ghi nhớ đăng nhập</label></div>}{message && <div className="access-error">{message}</div>}<button className="primary-btn auth-submit" disabled={authWorking}>{isRegister ? "Tạo tài khoản" : "Đăng nhập"}</button><div className="auth-divider"><span />hoặc<span /></div><button type="button" className="auth-alt" disabled={authWorking} onClick={() => { setAuthMode(isRegister ? "login" : "register"); setMessage(""); }}>{isRegister ? "Quay lại đăng nhập" : "Tạo tài khoản mới"}</button></div><footer><strong>Dự án web cá nhân</strong><span>Nhật ký thực tập UEH</span><span>Thiết kế và phát triển bởi Sinh viên UEH</span></footer></form></div>;
  ...
  return <div className="app-shell"><aside className={menu ? "sidebar open" : "sidebar"}>
    <Link className="brand ueh-sidebar-logo" href="/"><span className="ueh-wordmark"><div><b>UEH</b><strong>UNIVERSITY</strong></div><span>Đại học Kinh tế<br />Thành phố Hồ Chí Minh<small>University of Economics<br />Ho Chi Minh City</small></span></span></Link>
    ...
    <div className="sidebar-foot"><span>●</span><div><b>Đồng bộ riêng tư</b><small>Tự động lưu dữ liệu</small></div></div>
  </aside>{menu && <button className="scrim" onClick={() => setMenu(false)} aria-label="Đóng menu" />}
  <main><header className="topbar"><button className="menu-btn" onClick={() => setMenu(true)} aria-label="Mở trình đơn">☰</button><div className="crumb">Không gian làm việc <span>/</span> {nav.find(n => n.view === view)?.label}</div><div className={status === "saved" ? "save saved" : status === "error" ? "save error" : "save"}><i />{status === "saved" ? "Đã lưu" : status === "error" ? "Lỗi lưu dữ liệu" : "Đang lưu…"}</div><span className="account-name">{user?.username}</span><span className="user-avatar">{initials || "LH"}</span><button className="logout-btn" onClick={() => void logout()}>Đăng xuất</button></header>
  <div className="page">{view === "dashboard" && <Dashboard state={state} update={update} />}{view === "logs" && <Logs state={state} update={update} />}{view === "plan" && <Plan state={state} update={update} />}{view === "activities" && <Activities state={state} update={update} />}{view === "conclusion" && <ConclusionView state={state} update={update} />}{view === "compliance" && <Compliance state={state} />}{view === "settings" && <Settings state={state} setState={setState} />}{["references", "appendices", "preview"].includes(view) && <Soon view={view} />}</div><footer className="app-footer"><span>Dự án web cá nhân · Nhật ký thực tập UEH</span><nav><span>Điều khoản sử dụng</span><span>Chính sách bảo mật</span><span>Hướng dẫn</span></nav></footer></main></div>;
}
...
function DashboardSchedule({ state, update }: { state: AppState; update: (fn: (s: AppState) => AppState) => void }) {
  const setDate = (key: "startDate" | "endDate", value: string) => update(s => {
    const internship = { ...s.internship, [key]: value };
    return { ...s, internship: { ...internship, totalWeeks: scheduleWeeks(internship.startDate, internship.endDate) } };
  });
  return <section className="dashboard-schedule"><div className="internship-info"><span>▦</span><div><small>THÔNG TIN THỰC TẬP</small><b>{state.internship.organization || "Chưa thiết lập đơn vị thực tập"}</b></div></div><Field label="Ngày bắt đầu"><input type="date" value={state.internship.startDate} max={state.internship.endDate || undefined} onChange={e => setDate("startDate", e.target.value)} /></Field><Field label="Ngày kết thúc"><input type="date" value={state.internship.endDate} min={state.internship.startDate || undefined} onChange={e => setDate("endDate", e.target.value)} /></Field><Link className="primary-btn add-log-btn" href="/nhat-ky">＋ Thêm nhật ký</Link></section>;
}
function Dashboard({ state, update }: { state: AppState; update: (fn: (s: AppState) => AppState) => void }) {
  ...
  <DashboardSchedule state={state} update={update} />
