...
export default function InternshipApp({ view }: { view: View }) {
  ...
  if (status === "locked") return <div className="access-page"><section className="access-intro"><div className="access-brand"><span className="brand-mark ueh-mark"><span>UEH</span></span><span><b>Nhật ký thực tập</b><small>KIỂM TOÁN 2026</small></span></div><div className="access-copy"><small>KHÔNG GIAN LÀM VIỆC CÁ NHÂN</small><h2>Ghi lại hành trình<br />thực tập của bạn.</h2><p>Lập kế hoạch, cập nhật công việc hằng ngày và hoàn thiện báo cáo thực tập trong một không gian thống nhất.</p><div className="access-highlights"><article><strong>Nhật ký</strong><span>Theo ngày và tuần</span></article><article><strong>Kế hoạch</strong><span>12 tuần thực tập</span></article><article><strong>Ngôn ngữ</strong><span>VI → EN chuyên ngành</span></article></div></div></section><form className="access-card" aria-busy={authWorking} onSubmit={e => { e.preventDefault(); void submitAuth(); }}><span className="brand-mark ueh-mark"><span>UEH</span></span><small>NHẬT KÝ THỰC TẬP UEH</small><div className="auth-tabs"><button type="button" disabled={authWorking} className={authMode === "login" ? "active" : ""} onClick={() => { setAuthMode("login"); setMessage(""); }}>Đăng nhập</button><button type="button" disabled={authWorking} className={authMode === "register" ? "active" : ""} onClick={() => { setAuthMode("register"); setMessage(""); }}>Đăng ký</button></div><h1>{authMode === "login" ? "Chào mừng bạn quay lại" : "Tạo tài khoản mới"}</h1><p>{authMode === "login" ? "Đăng nhập để mở nhật ký của bạn." : "Mỗi tài khoản có một không gian nhật ký riêng."}</p>{authMode === "register" && <label><span>Họ và tên</span><input disabled={authWorking} autoComplete="name" value={fullName} onChange={e => setFullName(e.target.value)} minLength={2} maxLength={80} required /></label>}<label><span>Tên đăng nhập</span><input disabled={authWorking} autoComplete="username" value={username} onChange={e => setUsername(e.target.value)} minLength={3} maxLength={32} pattern="[A-Za-z0-9._-]+" required /></label><label><span>Mật khẩu</span><input disabled={authWorking} type="password" autoComplete={authMode === "login" ? "current-password" : "new-password"} value={password} onChange={e => setPassword(e.target.value)} minLength={6} maxLength={128} required /></label>{message && <div className="access-error">{message}</div>}<button className={`primary-btn auth-submit${authWorking ? " busy" : ""}`} disabled={authWorking}>{authMode === "login" ? "Đăng nhập" : "Tạo tài khoản"}</button></form></div>;
